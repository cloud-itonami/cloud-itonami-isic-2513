(ns steamgenmfg.sim
  ...
  production quantity, a proposal that tries to ACTUATE fabrication/
  welding/pressure-vessel-assembly-line equipment directly (permanently
  blocked, no override), a proposal that tries to self-issue an ASME
  BPVC pressure-vessel safety-certification mark (permanently blocked,
  no override), a double-schedule of the same maintenance window, a
  production-batch patch with a fabricated product type, a production-
  batch patch with an implausible hydrotest-pressure-bar reading, and a
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on fab-001 (verified, registered, fabrication line -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on fab-001 (always escalates -- approve) ==")
    ...
    (println (exec-op actor "t6"
                       {:op :actuate-fabrication-line :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on bench-002 (UNVERIFIED/unregistered hydrotest bench -> HARD hold) ==")
    ...
    (println "== coordinate-shipment ship-3 on batch-002 (10 units would exceed quantity 120 vs shipped 115 -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on fab-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible hydrotest-pressure-bar -> HARD hold ==")
    ...
    (println "== log-production-batch batch-001 attempting to self-issue a pressure-vessel safety certification -> HARD hold, PERMANENT ==")
