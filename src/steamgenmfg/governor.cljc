(ns steamgenmfg.governor
  "Steam Generator Plant Operations Governor -- the independent
  compliance layer that earns the SteamGeneratorAdvisor the right to
  ...
  draft-schedule) fabrication-line equipment, whether a proposal
  secretly tries to self-issue an ASME BPVC pressure-vessel safety-
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:fabrication-line/
                                       actuate` or `:welding-line/run`)
                                       is the 'direct fabrication-line-
    ...
    4. Equipment-actuate blocked   -- for `:schedule-maintenance`, does
                                       ...
                                       fabrication/welding/pressure-
                                       vessel-assembly-line equipment is
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       attempting to self-issue an ASME
                                       Boiler and Pressure Vessel Code
                                       (BPVC) Section I 'S' stamp
                                       certification or a National
                                       Board of Boiler and Pressure
                                       Vessel Inspectors (NBBI)
                                       registration -- an authority
   ...
   10. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       closed known set
                                       (`steamgenmfg.registry/product-
                                       type-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated product type through
                                       (this includes any attempt to
                                       log a central-heating hot-water-
                                       boiler product type, which ISIC
                                       2513 explicitly excludes).
   11. Invalid hydrotest-pressure-
       bar                          -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:hydrotest-pressure-bar` that is
                                       not a physically plausible
                                       hydrostatic pressure-test reading
                                       (`steamgenmfg.registry/
                                       hydrotest-pressure-bar-valid?`),
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [steamgenmfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct fabrication-
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct fabrication-line-equipment control, a
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は製缶・溶接・圧力容器組立設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate fabrication/welding/pressure-vessel-assembly-line
  equipment -- this actor may only ever propose/schedule a DRAFT
  maintenance window, never actuate the equipment directly. No
  override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-equipment? (:value proposal))))
    [{:rule :equipment-actuate-blocked
      :detail "製缶・溶接・圧力容器組立・水圧試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue an ASME Boiler and Pressure Vessel Code (BPVC) Section
  I 'S' stamp certification or a National Board of Boiler and Pressure
  Vessel Inspectors (NBBI) registration -- an authority exclusively
  reserved to the accredited certification body, never this actor. No
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "ASME BPVC Section I 'S'スタンプ認証・National Board(NBBI)登録の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-hydrotest-pressure-bar-violations
  "For `:log-production-batch`, if the patch declares a
  `:hydrotest-pressure-bar` that is not a physically plausible
  hydrostatic pressure-test reading, reject rather than let
  fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [bar (:hydrotest-pressure-bar (:value proposal))]
      (when (and (some? bar) (not (registry/hydrotest-pressure-bar-valid? bar)))
        [{:rule :invalid-hydrotest-pressure-bar
          :detail (str bar "bar は物理的に妥当な水圧試験値の範囲外")}]))))
...
(defn check
  "Censors a SteamGeneratorAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-hydrotest-pressure-bar-violations request proposal)
