(ns steamgenmfg.advisor
  "SteamGeneratorAdvisor -- the *contained intelligence node* for the
  industrial steam-generator/boiler plant-operations coordination
  actor.

  It normalizes production-batch patches (product-type/hydrotest-
  pressure-bar/quantity/defect-rate), drafts a fabrication/welding/
  pressure-vessel-assembly/hydrotest-bench-equipment maintenance
  scheduling proposal against a piece of equipment, drafts a safety-
  concern flag, and drafts an outbound steam-generator/boiler shipment
  ...
  a real fabrication-line-equipment actuation, freight dispatch, or
  safety certification -- see README `What this actor does NOT do`.
  Every output is censored downstream by `steamgenmfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `steamgenmfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct fabrication-
  ...
  `steamgenmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  hydrotest-pressure-bar, quantity, defect-rate or verification status.
...
(defn- schedule-maintenance
  "Draft a fabrication/welding/pressure-vessel-assembly/hydrotest-
  bench-equipment maintenance-window scheduling proposal against a
  piece of equipment. The advisor reports what it can see (equipment
  verified?/registered?) in its rationale, but `steamgenmfg.governor`
...
(defn- flag-safety-concern
  "Draft an equipment-safety/pressure-vessel-safety concern. ALWAYS
  ...
  `steamgenmfg.phase`: no phase ever adds this op to a phase's `:auto`
  set; `steamgenmfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound steam-generator/boiler shipment coordination
  ...
  `steamgenmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは工業用蒸気発生器・ボイラー(火管ボイラー・水管ボイラー・"
       "廃熱回収ボイラー・排熱回収蒸気発生器等の工業用蒸気発生設備、"
       "セントラルヒーティング用温水ボイラーは除く)製造工場プラント運用"
       "コーディネーターの助言者です。"
       ...
       "製缶・溶接・圧力容器組立・水圧試験等の設備の直接操作(actuate)を"
       "絶対に提案してはいけません(この actor は提案のみを行い、実行は"
       "一切行いません)。ASME BPVC Section I 'S'スタンプ認証や"
       "National Board(NBBI)登録を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `steamgenmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :steamgenmfg-advisor-proposal
   :op         (:op request)
