(ns steamgenmfg.operation
  "SteamGeneratorOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (SteamGeneratorAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Steam Generator Plant Operations Governor (:govern) and the rollout
  ...
  (:require [langgraph.graph :as g]
            ...
            [steamgenmfg.advisor :as advisor]
...
(defn build
  "Compiles a SteamGeneratorOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `steamgenmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SteamGeneratorAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Steam Generator Plant Operations Governor -- independent censor.
      (g/add-node :govern
