(ns steamgenmfg.registry
  "Pure-function domain logic for the industrial steam-generator/boiler
  (fire-tube boilers, water-tube boilers, waste-heat-recovery boilers,
  heat-recovery steam generators) plant-operations coordination actor --
  equipment/batch verification, shipment-quantity recompute, product-
  type validation, hydrotest-pressure plausibility validation,
  defect-rate plausibility validation, and draft maintenance-schedule/
  shipment-coordination record construction.

  Per docs/adr/0001-architecture.md Decision 1: this vertical has NO
  pre-existing `kotoba-lang/steamgenmfg`-style capability library to
  wrap (verified: no such repo exists). The domain logic therefore
  lives here as pure functions, re-verified INDEPENDENTLY by
  `steamgenmfg.governor` -- the same 'ground truth, not self-report'
  discipline every sibling actor's own registry establishes (e.g.
  `fluidpowermfg.registry/shipment-quantity-exceeded?` from
  `cloud-itonami-isic-2812`, and `ovenfurnacemfg.registry` from
  `cloud-itonami-isic-2815`): never trust a proposal's own self-
  reported quantity/status when the inputs needed to recompute it
  ...
  coordinated shipment), not the act of actuating fabrication/welding/
  pressure-vessel-assembly-line equipment or dispatching a real freight
  carrier, and never the act of issuing an ASME BPVC pressure-vessel
  safety-certification mark (this actor NEVER does any of those -- see
  README `What this actor does NOT do`).

  SCOPE: ISIC 2513 covers manufacture of steam generators, EXCEPT
  central heating hot water boilers -- fabrication, welding, pressure-
  vessel-assembly, and hydrotest lines producing finished fire-tube
  boilers, water-tube boilers, waste-heat-recovery boilers and heat-
  recovery steam generators (industrial/utility-scale steam-raising
  pressure vessels, distinct from household central-heating hot-water
  boilers, which this class explicitly excludes). This actor
  ...
  flagging, shipment coordination) -- it never touches the fabrication/
  welding/pressure-vessel-assembly-line equipment directly, and it
  never stands in for the certification body that issues ASME Boiler
  and Pressure Vessel Code (BPVC) Section I 'S' stamp certifications or
...
(def valid-product-types
  ...
  through. Deliberately excludes central heating hot water boilers --
  ISIC 2513 explicitly excludes that product from this class."
  #{:fire-tube-boiler :water-tube-boiler :waste-heat-recovery-boiler
    :heat-recovery-steam-generator})

(def hydrotest-pressure-bar-min
  "Physical floor for a batch's own hydrostatic pressure-test reading,
  in bar. A measured pressure-vessel hydrotest reading is never
  negative."
  0.0)

(def hydrotest-pressure-bar-max
  "Physical ceiling for a batch's own hydrostatic pressure-test
  reading, in bar. Informed by real industrial steam-generator/boiler
  test practice: small fire-tube boilers commonly operate at working
  ...
  and utility/supercritical power boilers can reach roughly 250-300 bar
  design pressure (ultra-supercritical units up to ~350 bar); ASME
  BPVC Section I hydrostatic testing is typically performed at 1.5x
  the maximum allowable working pressure (MAWP) -- this ceiling
  generously bounds well above any real routine hydrotest reading on
  any standardized class of industrial steam generator; a reading
  above it is implausible sensor/QC data, not a real measurement."
  600.0)

(def defect-rate-min-percent
  "Physical floor for a batch's own fabrication/welding/assembly/
  hydrotest defect-rate reading (zero defects is the best possible
  outcome, never negative)."
  0.0)

(def defect-rate-max-percent
  "Physical ceiling for a batch's own fabrication/welding/assembly/
  hydrotest defect-rate reading -- a batch cannot reject more than 100%
...
(defn batch-verified?
  ...
  (i.e. its product-type/hydrotest-pressure-bar/quantity/defect-rate
...
(defn hydrotest-pressure-bar-valid?
  "Is `bar` a physically plausible hydrostatic pressure-test reading,
  in bar? Rejects nil, non-numbers, negative values, and values beyond
  `hydrotest-pressure-bar-max` -- a fabricated or sensor-error
  reading, never let through as a real test-result fact."
  [bar]
  (and (number? bar)
       (>= (double bar) hydrotest-pressure-bar-min)
       (<= (double bar) hydrotest-pressure-bar-max)))

(defn defect-rate-valid?
  "Is `percent` a physically plausible batch fabrication/welding/
  assembly/hydrotest defect-rate reading? Rejects nil, non-numbers,
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER an ASME BPVC pressure-vessel safety-certification
  mark -- this actor is never the certification authority (see README
...
(defn register-maintenance
  ...
  fabrication/welding/pressure-vessel-assembly/hydrotest-bench-
  equipment maintenance window against a verified, registered piece of
  equipment. Pure function -- does not actuate the fabrication-line
  equipment or execute any maintenance; it builds the RECORD a plant
  coordinator would keep. `steamgenmfg.governor` independently re-
  verifies the equipment's own verified/registered ground truth, and
  permanently blocks any attempt to directly actuate the equipment
...
(defn register-shipment
  ...
  outbound steam-generator/boiler shipment against a verified,
  ...
  keep. `steamgenmfg.governor` independently re-verifies the
