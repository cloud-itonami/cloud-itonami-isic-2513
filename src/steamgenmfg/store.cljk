(ns steamgenmfg.store
  "SSoT for the industrial steam-generator/boiler (fire-tube boilers,
  water-tube boilers, waste-heat-recovery boilers, heat-recovery steam
  generators) plant-operations coordination actor, behind a `Store`
  protocol so the backend is a swap, not a rewrite -- the same seam
  every `cloud-itonami-isic-*` actor in this fleet uses.

  Scope note: like its siblings (`cloud-itonami-isic-2812`'s own
  `fluidpowermfg.store`, `cloud-itonami-isic-2815`'s own
  `ovenfurnacemfg.store`), this build ships a single `MemStore` backend
  ...
  is self-contained (no external steam-generator-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/hydrotest-pressure-bar/
    ...
    - `equipment`         -- a fabrication-line/welding-line/pressure-
                             vessel-assembly-line/hydrotest-bench
    ...
    - `maintenance`       -- a scheduled fabrication/welding/pressure-
                             vessel-assembly/hydrotest-bench-equipment
                             maintenance-window DRAFT against a piece
                             of equipment (`steamgenmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound steam-generator/boiler
                             shipment DRAFT (`steamgenmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (steamgenmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (steamgenmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :fire-tube-boiler
                :model "FB-A600" :hydrotest-pressure-bar 25.0
                :quantity-units 400.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :water-tube-boiler
                :model "WT-T12" :hydrotest-pressure-bar 150.0
                :quantity-units 120.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 115.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :heat-recovery-steam-generator
                :model "HRSG-R25" :hydrotest-pressure-bar 60.0
                :quantity-units 800.0 :defect-rate-percent 1.5
                :verified? false :registered? false
                :shipped-units 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"fab-001" {:id "fab-001" :kind :fabrication-line
              :verified? true :registered? true
              :last-maintenance-date "2026-05-01"}
   "bench-002" {:id "bench-002" :kind :hydrotest-bench
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `steamgenmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `steamgenmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  fabrication-line unit (schedulable for maintenance), one
  UNVERIFIED/unregistered hydrotest-bench unit (blocks any maintenance
