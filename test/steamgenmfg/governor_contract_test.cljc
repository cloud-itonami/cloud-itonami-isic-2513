(ns steamgenmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control fabrication-line equipment
  directly... does NOT authorize or execute maintenance/equipment
  operations... does NOT self-issue an ASME BPVC pressure-vessel
  safety-certification mark') implemented faithfully. The single
  invariant under test:

    SteamGeneratorAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Steam Generator Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :fire-tube-boiler}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :fire-tube-boiler (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-fabrication-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the ASME BPVC pressure-vessel safety-certification authority"
    (let [[db actor] (fresh)
...
(deftest central-heating-hot-water-boiler-product-type-is-held
  (testing "ISIC 2513 explicitly excludes central heating hot water boilers -- attempting to log one as a product-type is treated the same as any other unrecognized/fabricated type"
    (let [[db actor] (fresh)
          res (exec-op actor "t10c" {:op :log-production-batch :effect :propose :subject "batch-001"
                                     :patch {:product-type :central-heating-hot-water-boiler}} coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:invalid-product-type} (-> (store/ledger db) last :basis)))
      (is (not= :central-heating-hot-water-boiler (:product-type (store/batch db "batch-001")))))))

(deftest invalid-hydrotest-pressure-bar-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:hydrotest-pressure-bar 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-hydrotest-pressure-bar} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:hydrotest-pressure-bar (store/batch db "batch-001"))) "fabricated hydrotest-pressure-bar never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "fab-001" :severity :moderate
                                            :description "pressure vessel weld anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "fab-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
