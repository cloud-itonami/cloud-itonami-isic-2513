(ns steamgenmfg.operation-test
  "Smoke tests for the compiled SteamGeneratorOperationActor graph
  itself (build + one happy path per op). The governor's full rule
  contract (HARD holds, escalation, phase gating) is exercised in
  `steamgenmfg.governor-contract-test`; the Store contract in
  `steamgenmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "SteamGeneratorOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "fab-001" :severity :moderate :description "pressure vessel weld anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
