(ns steamgenmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [steamgenmfg.registry :as r]))
...
(deftest missing-quantity-is-not-flagged-exceeded
  ...
  (is (false? (r/shipment-quantity-exceeded? {:quantity-units 120.0} nil))))

;; ----------------------------- product-type-valid? -----------------------------

(deftest known-product-types-are-valid
  (doseq [pt [:fire-tube-boiler :water-tube-boiler :waste-heat-recovery-boiler
...
(deftest central-heating-hot-water-boiler-is-invalid
  (is (not (r/product-type-valid? :central-heating-hot-water-boiler))
      "ISIC 2513 explicitly excludes central heating hot water boilers -- never a valid product type here"))

;; ----------------------------- hydrotest-pressure-bar-valid? -----------------------------

(deftest typical-hydrotest-pressure-bar-is-valid
  (is (r/hydrotest-pressure-bar-valid? 0.0))
  (is (r/hydrotest-pressure-bar-valid? 25.0))
  (is (r/hydrotest-pressure-bar-valid? 150.0))
  (is (r/hydrotest-pressure-bar-valid? 600.0)))

(deftest negative-hydrotest-pressure-bar-is-invalid
  (is (not (r/hydrotest-pressure-bar-valid? -1.0))))

(deftest excessive-hydrotest-pressure-bar-is-invalid
  (is (not (r/hydrotest-pressure-bar-valid? 999999.0)))
  (is (not (r/hydrotest-pressure-bar-valid? 600.01))))

(deftest non-numeric-or-missing-hydrotest-pressure-bar-is-invalid
  (is (not (r/hydrotest-pressure-bar-valid? nil)))
  (is (not (r/hydrotest-pressure-bar-valid? "150.0"))))
